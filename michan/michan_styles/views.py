...
from .models import Comodities,vidoes
from .forms import ComoditiesForm
...
def gallery(request):
    ...
    gallery_pictures = Comodities.objects.all()
...
def New(request): 
    # Query the database for all pictures
    New_pictures = Comodities.objects.filter(category='New')
    paginator = Paginator(New_pictures, 20)  # 20 images per page
     # Implement pagination
    page_number = request.GET.get('page')  # Get the current page number from query parameters
    page_obj = paginator.get_page(page_number)  # Get the corresponding page object
...
def Men(request): 
    # Query the database for all pictures
    men_pictures = Comodities.objects.filter(category='Men')
...
def Women(request): 
    # Query the database for all pictures
    women_pictures = Comodities.objects.filter(category='Women')
...
def Accessories(request): 
    # Query the database for all pictures
    Accessories_pictures = Comodities.objects.filter(category='Accessories')
    paginator = Paginator(Accessories_pictures, 20)  # 20 images per page
     # Implement pagination
    page_number = request.GET.get('page')  # Get the current page number from query parameters
    page_obj = paginator.get_page(page_number)  # Get the corresponding page object
...
def Collections(request): 
    # Query the database for all pictures
    Collections_pictures = Comodities.objects.filter(category='Collections')
    paginator = Paginator(Collections_pictures, 20)  # 20 images per page
     # Implement pagination
    page_number = request.GET.get('page')  # Get the current page number from query parameters
    page_obj = paginator.get_page(page_number)  # Get the corresponding page object
    return render(request, 'Collections.html', {'page_obj': page_obj})   

#gallery view
def Store(request): 
    # Query the database for all pictures
    store_pictures = Comodities.objects.filter(category='Store')
...
#gallery view
def Bags_and_Leather_goods(request): 
    # Query the database for all pictures
    Bags_and_Leather_goods_pictures = Comodities.objects.filter(category='Bags & Leather Goods')
    paginator = Paginator( Bags_and_Leather_goods_pictures, 20)  # 20 images per page
     # Implement pagination
    page_number = request.GET.get('page')  # Get the current page number from query parameters
    page_obj = paginator.get_page(page_number)  # Get the corresponding page object
    return render(request, 'Bags_and_Leather_pictures.html', {'page_obj': page_obj})  
